...
- Semantic properties for broad laws: RMS declares input spaces, oracles, evidence, replayable counterexamples, and the realization claim. Fixed corpora, finite exhaustive checks, generated properties, and coverage fuzzers remain distinct; every realization names an exact runner, and generated or exhaustive strategies also name an exact generator.
- Execution-derived proof: trace producers call the real transition-record path, property runners execute their declared generators, operations, and oracles, and strict audit regenerates deterministic smoke proofs from committed code before accepting them.
...
rms init . \
  ...
  --purpose "Describe the existing system" \
  --adopt
```
...
Generated inspectable bindings declare inner structure in `implementation.yaml`: representation, binding-native message envelopes, transition output, an exact transition-record function, parser, adapter, journal, timeline projection, replay bundle, first-bad-transition, trace-producer, and trace-evidence roles, plus a domain-named machine with an explicit mode and state variants. Transition outputs keep expected failures in a typed rejection channel. Trace producers execute the real transition-record function and serialize returned records; they do not copy declarations into static YAML. `rms trace run --record` validates and records generated bundles, normal trace runs compare fresh execution with committed evidence, and `rms verify` checks recorded bundles after native verification. Role types use a semantic domain prefix, not the module slug: role, binding, and surface suffixes such as `rules`, `engine`, `adapter`, `cli`, `web`, `js`, `rust`, and `swift` are stripped before appending `Machine`, `State`, `Command`, `Event`, `Effect`, `EffectResult`, `Reply`, `Rejection`, `Transition`, and `TransitionRecord`. Prefer the `add-capability` default child paths unless the user supplied better product/domain names; do not invent `-rules`, `-adapter`, `-cli`, or `-web` child names just to describe RMS roles.

RMS also closes the layers explicitly. Every implemented public command, query, or capability has one `architecture.public_behavior_bindings` path from its contract through a discharging semantic function into classified machine inputs and outputs. Every implemented required capability has one `architecture.dependency_behavior_bindings` path from an exact local consumer to a matching RMS provider contract or an explicit external boundary. These are canonical declarations, not name-matching heuristics.

Semantic properties live above language-specific property-test libraries. A law says what must always hold; a property says which inputs to generate and which oracle judges them; a Rust, JS, Swift, Python, or executable binding decides how to run that property. Revise them through `rms spec apply` with `properties.add/set/remove`. Every executable realization names an exact `path#symbol` runner. Generated-property and deterministic-exhaustive strategies also name a generator; the runner must call that generator, execute the declared semantic operation, and apply a binding-native oracle. A fixed collection remains a corpus regardless of its label. RMS executes each smoke realization independently, even when commands are shared.
...
Record and compare execution-derived transition evidence with:

```bash
rms trace run ./my-system/modules/widget/implementation.yaml --profile smoke --record
rms trace run ./my-system/modules/widget/implementation.yaml --profile smoke
```

Strict audit reruns smoke trace and property proofs from the committed implementation, rebuilds reusable packages in a temporary directory, compares any existing package output, and fails if a proof command mutates production files.

For app, tool, CLI, local-first reference app, runnable, or smoke-test intents, the boundary child should expose a declared runnable surface. A runnable surface adapts outside input into declared RMS commands, may render or execute declared boundary effects, and must not reimplement domain decisions or call private module internals. Inspectable surfaces delegate to an exact callable so RMS can prove the live app reaches the machine. Boundary IO is represented as declared effects, typed results, atomic protocols, and dedicated executor ownership rather than hidden inside the runnable controller or parser. The surface also names a product usage document and a smoke command from the implementation binding; `rms verify` runs that smoke command. Browser, CLI, mobile UI, desktop UI, HTTP route, batch command, and opaque executable are bindings over that same semantic role.
...
Composite modules do not need to duplicate child property suites. A parent invariant may use `verification.delegations` to point to a contained provider law and property through a public export; `rms compose` verifies every link and the parent evidence.
...
The explorer binds only to loopback and projects one canonical cross-layer graph: modules, contracts, implementation machines, semantic functions, behavior bindings, effects, traces, evidence, and source references. Six focused views cover system topology, behavior paths, machines, proof chains, gap triage, and execution-derived debug timelines. Completeness is shape-aware: each obligation is `satisfied`, `required-gap`, `unresolved-link`, `recommendation`, or `not-applicable`, so a pure engine is not criticized for lacking boundary IO and a composite is not given a fake machine. The UI is experimental and read-only; stable deep links, search, source-backed inspection, and live refresh never become a second source of semantics. Use `--no-open` for headless environments and `--port <port>` to choose the loopback port.
...
- Use `rms init` for empty new projects. Use `rms init --adopt` when an existing repository already owns documents that RMS must preserve.
