...
5. Make each nightly runner honor `RMS_HUNT_SEED` and `RMS_HUNT_BUDGET_SECONDS`, then write `rms/hunt-lane-result/v0.1` to the exact path in `RMS_HUNT_OUTPUT`. A minimal passing result is:
