...
2. Extract a typed `rms/intent-model/v0.1` and use `rms next` for ownership routing. Use typed `rms design` only when module boundaries change; never infer topology from raw task wording. Build a bounded packet with `rms context`, attach an intentionally deferred implementation with `rms add-binding`, and use `rms plan` only after RMS has selected the owner.
