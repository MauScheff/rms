...
- Let semantic planning introduce a missing required capability by loading the exact task-named provider contract, using the correct `contracts.add` schema, and retaining that provider's v0.2 or v0.3 wrapper through candidate validation and apply. Let a composite add or repair an export by declaring the exact contained-child edge and contract evidence without reproducing provider semantics; `spec apply` materializes the provider artifact byte-for-byte, normalizes legacy planner contract rewrites into edge synchronizations, and rejects missing evidence, conflicting edges, unresolved children, and ambiguous provider contracts. Unrelated child contracts stay outside bounded planning context. Wrapper inference for required capabilities still requires one exact module-resolution binding; unresolved or ambiguous providers retain strict default validation.
