...
- Let semantic planning introduce a missing required capability by loading the exact task-named provider contract, using the correct `contracts.add` schema, and retaining that provider's v0.2 or v0.3 wrapper through candidate validation and apply. Let a composite add a new capability export from the exact contract of one contained child when the task names that capability; unrelated child contracts stay outside bounded planning context, and ambiguous providers remain unavailable. Wrapper inference requires one exact module-resolution binding; unresolved or ambiguous providers retain strict default validation.
