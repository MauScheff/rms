...
- Keep provider-extracted `new-module/design` intent from overriding an exact existing canonical owner when the task explicitly scopes maintenance to that module. Consumer and executor mentions remain participants, while explicit adoption and new-topology requests remain design work. Expose `--provider-timeout-seconds` on `rms next --ai` through the existing bounded provider cancellation path.
- Let semantic planning introduce a missing required capability by loading the exact task-named provider contract, using the correct `contracts.add` schema, and retaining that provider's v0.2 or v0.3 wrapper through candidate validation and apply. Wrapper inference requires one exact module-resolution binding; unresolved or ambiguous providers retain strict default validation.
