...
- Require transition-backed command and capability behavior to use transition-record observations while retaining invocation records for queries and non-transition boundary behavior. Stop invalid trace producers before execution, and defer `--record` writes until every selected producer passes so a failed trace run cannot replace committed evidence.
- Keep v0.2 probe CLI final-state and final-case expectations in RMS, project a schema-valid request to the adapter, and evaluate the returned transition trace against those expectations.
...
- Let semantic planning introduce or replace an exact required capability by declaring its identity, one same-change module-resolution binding, and contract evidence without reproducing provider semantics; `spec apply` discards any planner-owned body and materializes the unique provider export byte-for-byte. Reject missing or ambiguous providers and bindings, conflicting contract paths or versions, and missing evidence. Required-capability replacements without a newly declared provider sync remain consumer-owned. Let a composite add or repair an export through the same deterministic exact-copy model; unresolved children, ambiguous provider contracts, missing evidence, and conflicting edges fail closed. Unrelated provider contracts stay outside bounded planning context.
- Let receipt-gated implementation v0.2 migration inspect mixed executable bindings through exact `path#symbol` language selection. Add strict shell call-graph analysis, closed Python dispatch-table expansion, safe-facade authority closure, and a migration candidate seal that preserves the prior semantic revision while detecting later drift. Scope Python file-method inference to exact Python call names so Swift domain methods such as `readinessProjected` and `resolve` remain pure. Unknown, dynamic, or ambiguous shell calls still block migration.
