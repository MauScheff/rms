...
11. Do not introduce a state machine only because a record has a status field. Use one when legal commands, facts, or invariants depend on lifecycle order. After scaffolding, use `rms spec apply` for new laws, contracts, machine variants, transitions, semantic roles, public entrypoints, and evidence obligations rather than hand-creating architecture files. Use `rms machine apply` only for focused inner-machine edits after the semantic layer is correct.
...
13. Runnable app, UI, browser, and CLI surfaces must be declared as roles and must route through the declared public entrypoint, parser, adapter, or boundary machine. Do not put real behavior only in `public/app.*`, `src/cli.*`, or similar files while the RMS boundary remains disconnected.
