...
10. Implement inside the owning boundary and inside RMS-declared role files. Small private helpers inside pure role files must stay pure; IO belongs in declared adapter, port, or effect-executor roles as effects plus effect results.
    ...
    - Runnable surfaces such as `public/app.*`, `src/cli.*`, routes, mobile views, or executable entrypoints must be declared with `rms surface apply` or `rms spec apply` and route through the declared public entrypoint, parser, adapter, or boundary machine before reaching pure decisions. Simple runnable surfaces should stay thin and stateless unless the product intent has real lifecycle/order/session/retry/status/recovery/workflow semantics.
