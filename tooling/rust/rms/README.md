...
RMS-generated machine receipts under `.rms/evidence/` are disposable derived evidence and must remain untracked. New and adopted projects include that ignore rule. For an older managed project, `rms agent sync --target codex --root .` adds or refreshes the marked RMS ignore block and preserves project-owned ignore rules.
