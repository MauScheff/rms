...
- `config_init_writes_defaults_and_refuses_overwrite` verifies generated `.rms/config.yaml` parses as workbench config, writes provider, model, reasoning effort, and run-record defaults plus timeout guidance, and refuses overwrite unless forced.
