...
- The clean-room dogfood path copies the release binary into a temporary PATH install, initializes a fresh Tic Tac Toe project, refreshes Codex and Claude project-local guidance, runs `rms design`, scaffolds a recursive `add-capability-tree` tree, validates and composes it, asserts route output sends rules work to `tic-tac-toe-rules` and boundary parsing work to `tic-tac-toe-cli`, builds a context packet, verifies the composite rollup, initializes git, and runs `rms gate`.
